...
describe("Raid readiness", function()
  ...
  it("produces safe reports without live loot state", function()
    ...
    assert_true(safe:find("Addon version: " .. dibs.VERSION, 1, true) ~= nil)
  ...
  it("opens a selectable report window without dumping the report into chat", function()
    local dibs = setup({ guildLeader = true, inRaid = false })
    local before = #(_G.__dibsMessages or {})
    local shell, report = dibs.Readiness.OpenReport("detailed")
    assert_not_nil(shell)
    assert_true(report:find("Addon version: " .. dibs.VERSION, 1, true) ~= nil)
    assert_equal(before, #(_G.__dibsMessages or {}))
    assert_equal(report, shell.reportText)
    assert_not_nil(shell.reportEditor)
  end)
  ...
  it("supports the authorized readiness slash command", function()
    ...
    assert_not_nil(dibs.Readiness.state.reportShell)
    assert_equal(before, #(_G.__dibsMessages or {}))
  end)
end)
